...
def createTree():
    ...
    while True:
        ...
        item = input(
            f"Enter an integer value for the Left child of {temp.item}: ")
        if item.isdigit():
            temp.left = createBTNode(int(item))

        item = input(
            f"Enter an integer value for the Right child of {temp.item}: ")
        if item.isdigit():
...
def identical(tree1, tree2):
    ...
    def pre_order_traversal(node, option):
        global tree1_list
        global tree2_list

        if node is not None:
            if option:
                tree2_list.append(node.item)
            else:
                tree1_list.append(node.item)

            pre_order_traversal(node.left, option)
            pre_order_traversal(node.right, option)

    pre_order_traversal(tree1, 0)
    pre_order_traversal(tree2, 1)

    return True if tree1_list == tree2_list else False
